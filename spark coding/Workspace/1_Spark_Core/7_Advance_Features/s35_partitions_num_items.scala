...
object MyProgram {
    ...
    def sizeWithMapPartitions(sc: SparkContext, myDatasetDir: String): Unit = {
        ...
        val inputRDDSizes : RDD[Int] = inputRDD.mapPartitions( iter => Array(iter.size).iterator, true )
    ...
    def sizeWithMapPartitionsWithIndex(sc: SparkContext, myDatasetDir: String): Unit = {
        ...
        // 2. Operation T1: We use mapPartitions to see how many partitions are there
        val inputRDDSizes : RDD[(Int, Int)] = inputRDD.mapPartitionsWithIndex( (index: Int, iter: Iterator[String]) => Array((index, iter.size)).iterator , true )
    ...
    // ------------------------------------------
    val myWhatToDoWithThePartition: (Int, Iterator[String]) => Iterator[(Int, Int)] = (index: Int, iter: Iterator[String]) => {
        //1. We create the output variable with the content of currentAccumulator
        var res: Iterator[(Int, Int)] = Array((index, iter.size)).iterator
