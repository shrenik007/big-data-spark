...
object MyProgram {
    ...
    def myMain(sc: SparkContext): Unit = {
        // 1. Operation C1: Creation 'parallelize', so as to store the content of the collection into a RDD
        val inputRDD: RDD[(Int, String)] = sc.parallelize(Array((16, "A"), (5, "A"), (213, "A")))

        // 2. Operation T1: Transformation 'sortByKey', so as to get a new RDD ('sortedRDD') from inputRDD.
        val sortedRDD: RDD[(Int, String)] = inputRDD.sortByKey()

        //2. Operation A1: 'collect'.
        val resVAL: Array[(Int, String)] = sortedRDD.collect()
