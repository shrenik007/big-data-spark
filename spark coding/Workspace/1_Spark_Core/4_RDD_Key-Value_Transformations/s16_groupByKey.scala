...
object MyProgram {
    ...
    def myMain(sc: SparkContext): Unit = {
        ...
        val inputRDD: RDD[(Int, Int)] = sc.parallelize(Array((1, 2), (3, 8), (1, 4), (3, 4), (3, 6)))

        // 2. Operation T1: Transformation 'groupByKey', so as to get a new RDD ('groupedRDD') from inputRDD.
        val groupedRDD: RDD[(Int, Iterable[Int])] = inputRDD.groupByKey()

        //3. Operation A1: 'collect'.
        val resVAL: Array[(Int, Iterable[Int])]  = groupedRDD.collect()
