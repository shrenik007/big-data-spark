...
def my_model(spark, monitoring_dir, time_step_interval):
    ...
    # 8. Operation T5: We add a new column with values 0 or 1 randomly determined
    randomSDF = windowSDF.withColumn("newNumber", pyspark.sql.functions.when(pyspark.sql.functions.rand() > 0.5, 1).otherwise(0))

    # 9. Operation T6: We add a new column with the current timestamp
    final_inputSDF = randomSDF.withColumn("my_time2", pyspark.sql.functions.current_timestamp()) \
                              .withWatermark("my_time2", "0 seconds")

    # 10. Operation T7: We aggregate the data again
    solutionSDF = final_inputSDF.groupBy(pyspark.sql.functions.window("my_time2", my_frequency, my_frequency),
                                         final_inputSDF["newNumber"]
                                        )\
                                .count()

    # 11. Operation O1: We create the DataStreamWritter, to print by console the results in complete mode
    myDSW = solutionSDF.writeStream\
                       .format("console") \
                       .trigger(processingTime=my_frequency) \
                       .outputMode("append")

    # 12. We return the DataStreamWritter
    return myDSW
