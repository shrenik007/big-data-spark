...
def ex2(ssc, monitoring_dir, station_number):
    ...
    # 1. We read our DStream from monitoring_dir
    inputDStream = ssc.textFileStream(monitoring_dir).map(lambda line: process_line(line))
    inputDStream.cache()
    start_date = datetime.strptime('01/09/2016', '%d/%m/%Y')
    end_date = datetime.strptime('31/08/2017', '%d/%m/%Y')
    filtered_data = inputDStream.filter(lambda x: x[0] == station_number)
...
def ex3(ssc, monitoring_dir, station_number):
    inputDStream = ssc.textFileStream(monitoring_dir).map(lambda line: process_line(line))
    inputDStream.cache()
    filtered_station_data = inputDStream.filter(lambda x: x[0] == station_number)
