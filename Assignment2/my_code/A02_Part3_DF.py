...
def ex3(spark, my_dataset_dir, station_number):
    ...
    converted_time_df = filtered_data.withColumn("scheduled_time", to_timestamp("scheduled_time", "HH:mm:ss")).withColumn("expected_arrival_time", to_timestamp("expected_arrival_time", "HH:mm:ss"))
    conditional_df = converted_time_df.withColumn('behind', when(col("scheduled_time") < col("expected_arrival_time"), 1).otherwise(0))
    conditional_df = conditional_df.withColumn('ahead', when(col("scheduled_time") >= col("expected_arrival_time"), 1).otherwise(0))
    required_data = conditional_df.select("station_number", "behind", "ahead")
    column_sum = required_data.groupBy("station_number").agg(sum(required_data.behind), sum(required_data.ahead)).withColumnRenamed("sum(behind)", "behind").withColumnRenamed("sum(ahead)", "ahead")
...
if __name__ == '__main__':
    ...
    my_dataset_dir = "FileStore/tables/my_dataset_single_file/"
